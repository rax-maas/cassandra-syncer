...
Target.prototype.sync = function(file, callback) {
  var self = this;
  var hardlpath;
  var relativeFilePath = this._filePath(file);
  var directoryPath = path.resolve(path.join(this._temppath, DATA_DIR));
  ...
  mkdirp(path.dirname(hardlpath), function(err) {
    ...
    fsutil.mkdir(path.dirname(hardlpath), 0700, function(err) {
      if (err) {
