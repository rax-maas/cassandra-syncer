...
SyncMaster.prototype._syncCallback = function(err, file) {
  ...
  if (self._isSyncable(file)) {
    self._manifest.generate(function(err) {
      self._target.manifest.setManifest(self._manifest, function(err) {
...
SyncMaster.prototype._syncCallbackRemoval = function(err, file) {
  var self = this;
  ...
  self._target.manifest.remove(file);
  self._target.writeManifest(function(err) {
    self._target.removeFile(file);
  });
};
...
SyncMaster.prototype.run = function() {
  ...
  async.waterfall([
    ...
    function readTargetManifest(callback) {
      self._target.readManifest(function(err) {
        callback(err);
      });
    },
    ...
    function watch(callback) {
      self._watcher = stalker.watch(self._options.source, function(err, file) {
        ...
        self._syncCallbackRemoval(err, file);
      });
      callback();
    }
