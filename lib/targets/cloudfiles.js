...
function CloudfilesTarget(url, temppath, re, options) {
  ...
  if (options.config.CACHE_PATH) {
    opts.cache = {};
    opts.cache.cachePath = options.config.CACHE_PATH;
  }
...
CloudfilesTarget.prototype._storeFile = function(file, relativeFilePath, callback) {
  ...
  function iter() {
    retryCount++;
    self._copyFile(file, newpath, function(err) {
      if (err) {
        if (retryCount > MAX_RETRIES) {
          log.error(sprintf('Massive Failure copying %s.', file));
        } else {
          log.error(sprintf('Failed copying %s. Retrying with count %d', file, retryCount));
          iter();
        }
      } else {
        callback();
...
CloudfilesTarget.prototype.getStats = function(filename, callback) {
  var self = this,
      dataPath = path.join(DATA_DIR, filename);
  async.waterfall([
    function(callback) {
      self.client.setAuth(function(err) {
        callback(err);
    ...
    function(callback) {
      log.info(sprintf('getStats %s', dataPath));
      self.client.getFileInfo(self._bucket, dataPath, callback);
    }
  ], callback);
};
...
CloudfilesTarget.prototype.writeManifest = function(callback) {
  var self = this,
      ...
      localFile = path.join(this._options.config.CACHE_PATH, path.basename(file) + randstr(8));
