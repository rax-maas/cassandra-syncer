...
CloudfilesTarget.prototype._storeFile = function(file, relativeFilePath, callback) {
  ...
  log.msg('CloudfilesTarget: copying ' + this._filePath(file) + ' to ' + newpath);
...
CloudfilesTarget.prototype._findLatestManifest = function(callback) {
  ...
  self.client.setAuth(function(err) {
    ...
    self.client.getFiles(self._bucket, function(err, files) {
      if (err) {
        callback(err);
        return;
      }
...
CloudfilesTarget.prototype.initialize = function(callback) {
  var self = this;
  this.client.setAuth(function(err) {
    if (err) {
      callback(err);
      return;
    }
    self.client.createContainer(self._bucket, function(err) {
      callback(err);
    });
  });
};
...
CloudfilesTarget.prototype.writeManifest = function(callback) {
  ...
  fs.writeFile(localFile, self.manifest.serialize(), function(err) {
    ...
    self.client.addFile(self._bucket, {'remote': file, 'local': localFile}, function(err, uploaded) {
      if (err) {
        callback(err);
        return;
      }
